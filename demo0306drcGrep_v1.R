...
#library(zoo)
pdk = readLines("22FDX_Rev1.1_0.0_QA02 pdk.txt")
pdk2 <- grep("SINGULAR INSIDE ALSO", pdk, value = TRUE)
write.csv(x = pdk2, file = paste(format(Sys.time(), "%Y%m%d"), "_pdk2.csv", sep = "") )

pdk2_EXT <- grep("EXT", pdk2, value = TRUE)
write.csv(x = pdk2_EXT, file = paste(format(Sys.time(), "%Y%m%d"), "_pdk2_EXT.csv", sep = "") )

pdk2_ENC <- grep("ENC", pdk2, value = TRUE)
write.csv(x = pdk2_ENC, file = paste(format(Sys.time(), "%Y%m%d"), "_pdk2_ENC.csv", sep = "") )
###
