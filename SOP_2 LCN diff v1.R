...
#i1 = read.csv("130_LMS_WebDM_generation_template_130RFSOI_Rev1.0_0.0 ch2 3rd run v4.csv", header = TRUE, stringsAsFactors=FALSE)
#i2 = read.csv("LCN-002653 130G-LP (v27).csv", header = TRUE, stringsAsFactors=FALSE)
...
#af1 <- as.data.frame( i1_v1[ order(i1_v1[dim(i1_v1)[2]]), ]$ans )
#colnames(af1)[1] <- "ans"
#bf1 <- as.data.frame( i2_v1[ order(i2_v1[dim(i2_v1)[2]]), ]$ans )
#colnames(bf1)[1] <- "ans"
###bf is key
...
str(ans1)
ans1$check_all <- ifelse(ans1$check_bf==1 & ans1$check_af==0,"added",
ifelse(ans1$check_bf==0 & ans1$check_af==1,"re-updated",0)
)
...
#i1_v1$name1 <- paste( i1_v1$GDS.Number, i1_v1$GDS.Datatype, sep = ";", collapse = NULL )
